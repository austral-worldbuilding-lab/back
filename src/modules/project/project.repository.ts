...
export class ProjectRepository {
  ...
  async createFromProvocation(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      let parentProjectId: string | undefined = undefined;
      let parentProjectConfiguration: Prisma.JsonValue | null = null;
      ...
      // Build project name with parent context
      // TODO: when we have method to find initial project, replace `${parentProjectFromLink.project.name}: ...` for `${initialProject.name}: ...`
      const provocationTitle =
        provocationContent?.title || provocation.question;
      let projectName: string;

      if (parentProjectFromLink) {
        projectName = `${parentProjectFromLink.project.name}: ${provocationTitle}`;
      } else {
        // No parent project - name is required
        if (!createProjectFromProvocationDto.name) {
          throw new BadRequestException(
            'Project name is required when the provocation was not generated by any project.',
          );
        }
        projectName = createProjectFromProvocationDto.name;
      }
      const projectDescription =
        provocationContent?.description || provocation.question;

      // Determine dimensions, scales, and organizationId with validation
      let dimensions = createProjectFromProvocationDto.dimensions;
      let scales = createProjectFromProvocationDto.scales;
      let organizationId = createProjectFromProvocationDto.organizationId;

      // If parent project exists, validate provided values match parent
      if (parentProjectFromLink && parentProjectConfiguration) {
        ...
        if (scales && scales.length > 0) {
          const scalesMatch =
            ...
            scales.every(
              (scale, index) => scale === parentConfig.scales[index],
            );
      ...
      // Set values: use provided, inherit from parent, or use defaults
      if (!dimensions || dimensions.length === 0) {
        if (parentProjectConfiguration) {
          const parentConfig = this.parseToProjectConfiguration(
            parentProjectConfiguration,
          );
      ...
      // If organizationId not provided, inherit from parent project
      if (!organizationId) {
        if (parentProjectFromLink) {
          organizationId = parentProjectFromLink.project.organizationId;
        } else {
